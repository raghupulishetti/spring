...
public class UserDaoImpl extends JdbcDaoSupport implements UserDao {
	...
	public int registerUser(User user) {
		int count = getJdbcTemplate().update(SQLConstants.SQL_REGISTER_USER, user.getUserId(), user.getName(),
	...
	public int updateUserMobile(final Integer userId, final String mobile) {
		int count = getJdbcTemplate().update(SQLConstants.SQL_UPDATE_MOBILE, new PreparedStatementSetter() {// start of AIC
	...
	public int deleteUser(final Integer userId) {
		int count = getJdbcTemplate().update(new PreparedStatementCreator() {// start of AIC public
	...
	@Override
	public int deleteUser(final Integer userId) {
		int count = getJdbcTemplate().update((Connection con) -> {
			PreparedStatement pst = con.prepareStatement(SQLConstants.SQL_DELETE_USER);
			pst.setInt(1, userId);
			return pst;
	...
	@Override
	public int updateUserMobile(final Integer userId, final String mobile) {
		int count = getJdbcTemplate().update(SQLConstants.SQL_UPDATE_MOBILE, (PreparedStatement pst) -> {
			pst.setString(1, mobile);
			pst.setInt(2, userId);
		});
		return count;
